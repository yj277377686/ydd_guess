<template>
	<view class="contents">
		<view class="room_head">
			<view class="room_Title ui-cell"><image src="/static/img/left.png" class='left_img' @tap="go_index"></image>高级区</view>
		</view>
...
<script>
	...
	export default {
		...
		onShow() {  
			let that=this;
			this.intervalc = setInterval(function () {
			  that.getroom();
			}, 5000);
		},
		created () {
		...
		methods: {
			autoJoin(){
				...
				get('Croom/quickly_room',{},token).then(res=>{
					console.log(res);
					if(res.code==1){
			...
			startGame(rid,online,num){
				if(online>=num){
					...
					this.intervalc = null
					uni.navigateTo({
						url:'/pages/gameRoom/gameRoom?rid='+rid+'&qname=高级区'+'&status=3'
					})
				}
			},
			getroom(){
				let that=this;
				get('Aroom/room_lists',{initial:4}).then(res=>{
					console.log(res);
					if(res.code==1){
