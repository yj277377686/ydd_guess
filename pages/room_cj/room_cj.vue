<template>
	<view class="contents">
		<view class="room_head">
			<view class="room_Title ui-cell"><image src="/static/img/left.png" class='left_img' @tap="go_index"></image>初级区</view>
		</view>
		<view class="roomList answerBox ui-cell">
			<view class="listTop">
			<view class="roomItems room_two ui-cell">
	...
	export default {
		...
		methods: {
			autoJoin(){
				...
				get('Aroom/quickly_room',{},token).then(res=>{
					console.log(res);
					if(res.code==1){
			...
			startGame(rid,online,num){
				if(online>=num){
					...
					this.intervala = null
					uni.navigateTo({
						url:'/pages/gameRoom/gameRoom?rid='+rid+'&qname=初级区'+'&status=1'
					})
				}
			},
			getroom(){
				let that=this;
				get('Aroom/room_lists',{initial:2}).then(res=>{
					console.log(res);
					if(res.code==1){
