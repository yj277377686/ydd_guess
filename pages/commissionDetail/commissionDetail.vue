		  ...
		  <view class="detailBox" v-if="detail.length>0"> 
			<view class="recordItem ui-cell" v-for="(item, index) in detail" :key="index">
				...
				<view class="itemRight ui-cell">
					 <view class="rightMoney ui-cell green" v-if="item.change_type==1"><text></text>{{item.change_amount}}</view>
					 <view class="rightMoney ui-cell red" v-else><text></text>{{item.change_amount}}</view>
				</view>
			</view>
		  </view>
	...
	export default {
		...
		methods: {
			...
			getDetail(type){
				...
				post('user/accountLog',{source:4,type:type,page:1,limit:1000},tokens).then(res=>{
					console.log(res);
					if(res.code==1){
