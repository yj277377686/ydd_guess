	...
	<view class="contents">
		 <view class="xyBox ui-cell" v-if="boxflag" @tap="checkbox">
			 <view class="boxContent" @tap.stop='checkLbox'>
				 ...
				 <view class="xyContent"><rich-text :nodes="xyContent"></rich-text></view>
			 </view>
		 </view> 
		 <view class="topBox">
	     ...
	     <view class="loginBox">
				...
				<view class="titles ui-cell"><view class="lg">登录</view><view class="rg">/注册</view></view>
				<view class="loginInput ui-cell">
					<view class="inputTitle">+86</view>
					<input type="number" placeholder="请输入手机号码" class="inputname" maxlength='11' adjust-position="false"  @input="changePhone">
				</view>
				<view class="loginInput ui-cell">
					<image src="/static/img/lgcode.png" class="lgcode"></image>
					<input type="number" placeholder="请输入短信验证码" maxlength='4' adjust-position="false"  @input="changeCode" class="inputname">
				...
				<view class="zcBox ui-cell">
					...
					已阅读并同意<view class="fwze" @tap="checkXy(1)">《服务协议》</view>和<view class="fwze" @tap="checkXy(2)">《隐私政策》</view>
				</view>
				<view class="loginBtn ui-cell" @tap="sjLogin">登 录</view>
	     </view>
	</view>
</template>
			...
			sjLogin(){
				...
				if(!that.phone||that.phone.length!=11){
				  uni.showToast({
					...
					let code = that.code;
					post('Account/smsLogin/',{mobile:phone,code:code,client:4}).then(res=>{
						console.log(res);
						if(res.code==1){
